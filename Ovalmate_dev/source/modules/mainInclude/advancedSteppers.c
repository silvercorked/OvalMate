...
void ADVSTEPPERS_drawOval() {
	...
	while (k <= OVALLINESHALF) { // negate x result if doing negative side
		...
		STEPPERS_moveBothRelativeNoAccelNoBlock(relativeY > 0 ? -yPosSteps : yPosSteps, relativeX > 0 ? xPosSteps : -xPosSteps);
		...
		while(stepperX_p->status.running || stepperY_p->status.running);
		STEPPERS_moveRelativeNoAccelNoBlock(stepperX_p, lineSteps); // y * 2 == line width
		currY = -y;
		k += kInc;
		if (k == -1) {
			kInc = 1;
