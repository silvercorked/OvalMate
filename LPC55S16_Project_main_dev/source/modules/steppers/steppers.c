...
stepperMotor motorY = { .direction = 0, .netSteps = 0,
...
void (*stepperMotorXCallback[4])(stepperMotor*); // passed a bool arg "motorStopped" and motor arg "motor(X|Y)
void (*stepperMotorYCallback[4])(stepperMotor*); // passed a bool arg "motorStopped" and motor arg "motor(X|Y)
// End Variables
...
void blockUntilMotorJobComplete(which_motor_t whichMotor) {
	if (whichMotor == sMotorBoth)
		while (motorX.running || motorY.running);
	else if (whichMotor == sMotorX) {
...
uint8_t getMotorCallbackLength(which_motor_t whichMotor) {
	assert(whichMotor != sMotorBoth);
	void (**m)(stepperMotor*) = stepperMotorXCallback;
	if (whichMotor == sMotorY)
...
status_t addMotorCallback(which_motor_t whichMotor, void (*f)(stepperMotor*)) {
	if (whichMotor == sMotorBoth) {
		uint8_t xLen = getMotorCallbackLength(sMotorX);
		uint8_t yLen = getMotorCallbackLength(sMotorY);
		if (xLen >= 4 || yLen >= 4)
			return kStatus_Fail;
		else {
...
status_t removeMotorCallback(which_motor_t whichMotor, uint8_t index) {
	if (index > 3)
		return kStatus_Fail;
	if (whichMotor == sMotorX || whichMotor == sMotorBoth)
		stepperMotorXCallback[index] = NULL;
	if (whichMotor == sMotorY || whichMotor == sMotorBoth)
...
status_t setupStepperPWM(stepperMotor* motorP, uint32_t stepsToAssign, uint32_t frequency) {
